...
if [ $# -ne 3 ]
then
  echo "Please write : ./amplq.sh [nbprofiles] [nbtests] [nbtest3]"
else
  mkdir parsed
...
echo nbtest3 $nbtest3
  echo 'NetworkProfiles mdc mddash_3S mddash_9S' > parsed/amplqout.txt
    ...
    do
      ...
      do
        ...
        then
          ...
          echo `awk 'NR==3{print $1}' tmp.txt` >> tmp$k.txt
