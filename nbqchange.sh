...
echo nbtest3 $nbtest3
  ...
  do
    ...
    mean1=`awk 'NR>0{v+=$1;count++}END{print (v/2)/'$nbtest'}' tmp1.txt`
    mean2=`awk 'NR>0{v+=$1;count++}END{print (v/2)/'$nbtest3'}' tmp2.txt`
    mean3=`awk 'NR>0{v+=$1;count++}END{print (v/2)/'$nbtest'}' tmp3.txt`
